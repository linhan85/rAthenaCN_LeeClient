...
class LeeSkilldescriptTranslator:
	def __init__(self):
		self.leeCommon = LeeCommon()
		self.leeSkilldescriptLua = LeeSkilldescriptLua()
	...
	def doTranslate(self):
		...
		rePathPattern = r'^.*?/Patches/.*?/Resource/Original/data/luafiles514/lua files/skillinfoz/skilldescript\.(lua|lub)'.replace('/', os.path.sep)
		self.loadTranslate()

		# 找到所有需要汉化的 skilldescript 文件路径
		skilldescriptFilePathList = []
		for dirpath, _dirnames, filenames in os.walk(patchesDir):
			for filename in filenames:
				fullpath = os.path.normpath('%s/%s' % (dirpath, filename))
				if not (filename.lower().startswith('skilldescript')): continue
				if not re.match(rePathPattern, fullpath, re.I): continue
				skilldescriptFilePathList.append(fullpath)
				# print(fullpath)
		
		# 挨个处理并保存到对应的 Translated 目录中
		for skilldescriptFilePath in skilldescriptFilePathList:
			print('正在汉化, 请稍候: %s' % os.path.relpath(skilldescriptFilePath, leeClientDir))
			regex = r'(^.*?/Patches/.*?/Resource)/Original/(data/luafiles514/lua files/skillinfoz/skilldescript\.(lua|lub))'.replace('/', os.path.sep)
			match = re.search(regex, skilldescriptFilePath, re.MULTILINE | re.IGNORECASE | re.DOTALL)
			if match is None:
				self.leeCommon.exitWithMessage('无法确定翻译后的 skilldescript 文件存放位置, 程序终止')
			savePath = '%s/Translated/%s' % (match.group(1), match.group(2))
			# print(savePath)

			self.leeSkilldescriptLua.load(skilldescriptFilePath)
			for skillConstant in self.leeSkilldescriptLua.items():
				...
				if 'Description' in skillTranslateData:
					...
					self.leeSkilldescriptLua.setItemAttribute(skillConstant, 'Description', skillDescriptText)
			self.leeSkilldescriptLua.save(savePath)
