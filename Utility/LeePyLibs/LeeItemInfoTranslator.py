...
class LeeIteminfoTranslator(LeeBaseTranslator):
    ...
    def create(self, srcIteminfoPath, translateDBPath = None):
        ...
        for itemID in self.leeFileIO.items():
            unidentifiedDescriptionName = self.leeFileIO.getItemAttribute(itemID, 'unidentifiedDescriptionName')
            identifiedDescriptionName = self.leeFileIO.getItemAttribute(itemID, 'identifiedDescriptionName')
            self.translateMap[itemID] = {
                'unidentifiedDisplayName' : self.leeFileIO.getItemAttribute(itemID, 'unidentifiedDisplayName'),
                'unidentifiedDescriptionName' : [] if unidentifiedDescriptionName is None else unidentifiedDescriptionName.split('\r\n'),
                'identifiedDisplayName' : self.leeFileIO.getItemAttribute(itemID, 'identifiedDisplayName'),
    ...
    def translate(self, srcFilepath, dstFilepath):
        ...
        for itemID in self.leeFileIO.items():
            ...
            if not self.leeCommon.isEmpty(itemTranslateData['unidentifiedDisplayName']):
                self.leeFileIO.setItemAttribute(itemID, 'unidentifiedDisplayName', itemTranslateData['unidentifiedDisplayName'])
            if not self.leeCommon.isEmpty(itemTranslateData['unidentifiedDescriptionName']):
                self.leeFileIO.setItemAttribute(itemID, 'unidentifiedDescriptionName', '\r\n'.join(itemTranslateData['unidentifiedDescriptionName']))
            if not self.leeCommon.isEmpty(itemTranslateData['identifiedDisplayName']):
                self.leeFileIO.setItemAttribute(itemID, 'identifiedDisplayName', itemTranslateData['identifiedDisplayName'])
            if not self.leeCommon.isEmpty(itemTranslateData['identifiedDescriptionName']):
                self.leeFileIO.setItemAttribute(itemID, 'identifiedDescriptionName', '\r\n'.join(itemTranslateData['identifiedDescriptionName']))
