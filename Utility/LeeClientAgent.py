...
from Modules import LeeCommon
from Modules import LeeConstant
...
class LeeMenu:
	def __init__(self, main, patchManager):
		self.item_Main = main
	...
	def resetWorkshop(self):
		...
		'''
		try:
			self.patchManager.restorePatch()
	...
	def switchWorkshop(self, clientver):
		...
		if len(restoreInfo) > 0:
			...
			prompt = '是否立刻执行重置操作?'
			LeeCommon.simpleConfirm(lines, title, prompt, self, 'menus.resetWorkshop()')

		# 将对应的资源覆盖到 LeeClient 主目录
		print('正在切换版本, 请耐心等待...')
		if not self.patchManager.applyPatch(clientver):
	...
	def item_SwitchWorkshop(self):
		...
		LeeCommon.simpleMenu(menus, '切换仙境传说主程序的版本', '请选择你想切换到的版本', self, withcancel = True)
	...
	def item_ResetWorkshop(self):
		...
		if len(restoreInfo) > 0:
			...
			LeeCommon.exitWithMessage('您的客户端环境看起来十分干净, 不需要再进行清理了.')
	...
	def item_End(self):
		...
		当选择“退出程序”时执行
		'''
		LeeCommon.exitWithMessage('感谢您的使用, 再见')
		return
...
def main():
	...
	LeeCommon.simpleMenu(menus, title, prompt, LeeMenu(main, LeePatchManager(LeeCommon)))
